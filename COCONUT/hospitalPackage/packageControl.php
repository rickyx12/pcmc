...
public function searchItem($desc,$packageName,$packagePrice) {
...
echo "<Td>&nbsp;&nbsp;<a href='/COCONUT/hospitalPackage/chargesQTY.php?desc=$row[Description]_$row[chargesCode]_$row[Category]&packageName=$packageName&packagePrice=$packagePrice&unitcost=&Added='><font color=red>Add</font></a>&nbsp;&nbsp;</tD>";
echo "</tr>";
}
echo "</table>";
...
public function searchInventory($desc,$packageName,$packagePrice) {
...
$result = mysql_query("SELECT description,inventoryCode,upper(inventoryType) as inventoryType,unitcost,Added from inventory where description like '$desc%%%%%%%%' order by description asc ");
...
echo "<tr>";
echo "<td>&nbsp;".$row['description']."</tD>";
echo "<Td>&nbsp;&nbsp;<a href='/COCONUT/hospitalPackage/chargesQTY.php?desc=$row[description]_$row[inventoryCode]_$row[inventoryType]&packageName=$packageName&packagePrice=$packagePrice&unitcost=$row[unitcost]&Added=$row[Added]'><font color=red>Add</font></a>&nbsp;&nbsp;</tD>";
echo "</tr>";
}
echo "</table>";
...
public function searchDoctor($desc,$packageName,$packagePrice) {
...
echo "<tr>";
echo "<td>&nbsp;".$row['Name']."</tD>";
echo "<Td>&nbsp;&nbsp;<a href='/COCONUT/hospitalPackage/chargesQTY.php?desc=$row[Name]_$row[doctorCode]_PROFESSIONAL FEE&packageName=$packageName&packagePrice=$packagePrice&unitcost=&Added='><font color=red>Add</font></a>&nbsp;&nbsp;</tD>";
echo "</tr>";
}
echo "</table>";
...
public function addPackageInclude($packageName,$packageIncluded_desc,$packageIncluded_qty,$packagePrice,$unitcost,$Added) {
...
$sql="INSERT INTO hospitalPackage (packageName,packageIncluded_description,packageIncluded_qty,packagePrice,unitcost,Added)
VALUES
('".mysql_real_escape_string($packageName)."','".mysql_real_escape_string($packageIncluded_desc)."','$packageIncluded_qty','$packagePrice','$unitcost','$Added')";
...
echo  "window.location='http://".$this->getMyUrl()."/COCONUT/hospitalPackage/searchCharges.php?packageName=$packageName&packagePrice=$packagePrice';";
echo "</script>";
...
$result = mysql_query("SELECT inventoryCode,description,quantity FROM inventory WHERE (description like '$item%%%%%' or genericName like '$item%%%%%') and quantity > 0 and inventoryLocation = 'PHARMACY' and status not like 'DELETED%%%%%%' ");
