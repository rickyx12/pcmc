...
<table width='100%' border='0' cellpadding='0' cellspacing='0'>
  ...
  <tr>
    <td><div align='center' class='style1'>Deleted Test Result/s</div></td>
  </tr>
  <tr>
  ...
  <tr>
    <td bgcolor='#FFFFFF'><table width='100%' border='0' cellpadding='0' cellspacing='0'>
      <tr>
        ...
        <td height='20' bgcolor='#0066FF' class='table2Top2Bottom'><div align='center' class='style4'>&nbsp;Time Performed&nbsp;</div></td>
...
$b=0;
$bsql=mysql_query("SELECT registrationNo, itemNo, chargesCode, medtech, date, time, status FROM labSavedResult WHERE date='$fdate' AND status NOT LIKE '' ORDER BY time");
...
$timedel=substr($status[1], -8, 8);
$datedel=substr($status[1], -19, 10);
...
$description=$cuz->selectNow("patientCharges","description","itemNo",$bfetch['itemNo']);

$rm=$cuz->selectNow("registeredUser","completeName","username",$bfetch['medtech']);
if($rm==''){$realm=$bfetch['medtech'];}else{$realm=$rm;}

$csql=mysql_query("SELECT description, chargeBy, remarks FROM patientCharges WHERE itemNo='".$bfetch['itemNo']."'");
while($cfetch=mysql_fetch_array($csql)){$description=$cfetch['description'];$chargeBy=$cfetch['chargeBy'];$remarks=$cfetch['remarks'];}

$rc=$cuz->selectNow("registeredUser","completeName","username",$chargeBy);
if($rc==''){$realc=$chargeBy;}else{$realc=$rc;}
...
echo "
      <tr>
        ...
        <td class='table1Bottom1Left'><div align='center' class='style5'>&nbsp;".$bfetch['time']."&nbsp;</div></td>
