...
<br />
<br />
<div align='center' class='style2'>Item Deleted.</div>
";
$pendingDelete=$cuz->selectNow("patientCharges","pendingDelete","itemNo",$itemNo);
$separate=preg_split('[_]',$pendingDelete);
$cuz->editNow("patientCharges","itemNo",$itemNo,"status","DELETED_".$username."[".date("Y-m-d")."@".$cuz->getSynapseTime()."]");
$cuz->editNow("patientCharges","itemNo",$itemNo,"pendingDelete","DELETED_".$separate[1]."_".$separate[2]."_".$separate[3]);
echo "<META HTTP-EQUIV='Refresh'CONTENT='2;URL=http://".$cuz->getMyUrl()."/COCONUT/ADMIN/pendingDelete_update.php?registrationNo=$registrationNo&username=$username'>";
?>
</body>
</html>
