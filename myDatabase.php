...
echo "<input type=text name='$name' id='$name' value='$value' class='shortField'>";
}
...
public function addNewCharges($description,$examination,$category,$opd,$ward,$soloward,$semiprivate,$private,$username,$subCategory,$hmo) {
...
$sql="INSERT INTO availableCharges (Description,Service,Category,OPD,WARD,SOLOWARD,SEMIPRIVATE,PRIVATE,subCategory,HMO)
VALUES
('$description','$examination','$category','$opd','$ward','$soloward','$semiprivate','$private','$subCategory','$hmo')";
...
public function addNewRegistration($patientNo,$registrationNo,$bloodPressure,$temperature,$height,$weight,$company,$initialDiagnosis,$dateRegistered,$timeRegistered,$branch,$type,$room,$username,$casetype,$limit) {
...
$sql="INSERT INTO registrationDetails (patientNo,registrationNo,bloodPressure,temperature,height,weight,Company,initialDiagnosis,dateRegistered,timeRegistered,branch,type,room,PIN,registeredBy,casetype,LimitCASH,control_dateRegistered)
VALUES
('$patientNo','$registrationNo','".mysql_real_escape_string($bloodPressure)."','".mysql_real_escape_string($temperature)."','".mysql_real_escape_string($height)."','".mysql_real_escape_string($weight)."','$company','".mysql_real_escape_string($initialDiagnosis)."','$dateRegistered','$timeRegistered','$branch','$type','$room','00-000000000-0','$username','$casetype','$limit','".date("Y-m-d")."')";
...
$result = mysql_query("SELECT username,module FROM registeredUser where password='$password' ");
...
$this->patientRecord_phicType = $row['phicType'];
...
if($room[1] == "ER" || $room[1] == "OR/DR" || $room[1] == "SUITE" || $room[1] == "DIALYSIS") {
...
$result = mysql_query("SELECT Category,chargesCode,upper(Description) as Description,(".$room[1].") as sellingPrice,Service FROM availableCharges where 1=1 order by description asc ");
}else if( $charges == "laboratory" || $charges == "lab"  ) {
$result = mysql_query("SELECT Category,chargesCode,upper(Description) as Description,(".$room[1].") as sellingPrice,Service FROM availableCharges where 1=1 and Category = 'LABORATORY' order by description asc ");
}else if( $charges == "radiology" || $charges == "rad"  ) {
$result = mysql_query("SELECT Category,chargesCode,upper(Description) as Description,(".$room[1].") as sellingPrice,Service FROM availableCharges where 1=1 and Category = 'RADIOLOGY' order by description asc ");
}
else if( $charges == "nbs" || $charges == "NBS"  ) {
$result = mysql_query("SELECT Category,chargesCode,upper(Description) as Description,(".$room[1].") as sellingPrice,Service FROM availableCharges where 1=1 and Category = 'NBS' order by description asc ");
}
else {
$result = mysql_query("SELECT Category,chargesCode,upper(Description) as Description,(".$room[1].") as sellingPrice,Service FROM availableCharges where Description like '".mysql_real_escape_string($charges)."%%%%%%%' ");
}
...
$result = mysql_query("SELECT * FROM patientCharges where registrationNo = '$registrationNo' and status = 'UNPAID' and description like '$desc%%%%%%' order by description asc ");
}
...
$result = mysql_query("SELECT * FROM patientCharges where registrationNo = '$registrationNo' and title='$title' and (status = 'UNPAID' or status = 'Discharged') order by description asc ");
...
//echo "<Td>&nbsp;<img src='http://".$this->getMyUrl()."/COCONUT/myImages/locked1.jpeg' />&nbsp;</tD>";
echo "<Td>&nbsp;<a href='http://".$this->getMyUrl()."/COCONUT/patientProfile/discharged/deleteRoom.php?registrationNo=$registrationNo&itemNo=$row[itemNo]&chargesCode=$row[chargesCode]'><img src='http://".$this->getMyUrl()."/COCONUT/myImages/locked1.jpeg' />&nbsp;</a></tD>";
}else if( $row['batchNo'] == "package" ) {
...
echo "<Td>&nbsp;<a href='http://".$this->getMyUrl()."/COCONUT/Laboratory/deleteResult.php?registrationNo=$registrationNo&itemNo=$row[itemNo]'><img src='http://".$this->getMyUrl()."/COCONUT/myImages/locked1.jpeg' />&nbsp;</tD>";
}else {
...
echo "<td>&nbsp;<a href='http://".$this->getMyUrl()."/COCONUT/Results/clinicalChemistry/labTest_head.php?description=$row[description]&registrationNo=$registrationNo&itemNo=$row[itemNo]&branch=$row[branch]&labTest=".$this->getSubCategory($row['chargesCode'])."'><font class='data'>".$row['service']."</font></a>&nbsp;</td>";
}else if($row['title'] == "RADIOLOGY") {
echo "<td>&nbsp;<a href='http://".$this->getMyUrl()."/COCONUT/Reports/radiologyReport/radioReportSettings.php?description=$row[description]&registrationNo=$registrationNo&itemNo=$row[itemNo]&branch=$row[branch]'><font class='data'>".$row['service']."</font></a>&nbsp;</td>";
}else if($row['title'] == "PROFESSIONAL FEE") {
echo "<td>&nbsp;<a href='http://".$this->getMyUrl()."/COCONUT/Doctor/doctorModule/soap.php?registrationNo=$registrationNo&itemNo=$row[itemNo]&username=$username'><font class='data'>".$row['service']."</font></a>&nbsp;</td>";
...
$result = mysql_query("SELECT pc.*,rd.type,rd.dateUnregistered FROM patientCharges pc,registrationDetails rd where rd.registrationNo = '$registrationNo' and rd.registrationNo = pc.registrationNo and pc.inventoryFrom='$title' and pc.dateCharge='$selectedDate' and pc.paidVia ='Cash' and (pc.timeCharge between '$fromTimez' and '$toTimez') and pc.departmentStatus not like 'dispensedBy%%%%%' and status not like 'DELETED_%%%%%' order by pc.description asc ");
}else if( $module =="LABORATORY" || $module == "BLOODBANK" ) {
$result = mysql_query("SELECT pc.*,rd.* FROM patientCharges pc,registrationDetails rd where rd.registrationNo = '$registrationNo' and rd.registrationNo = pc.registrationNo and (pc.title='LABORATORY' or pc.title = 'BLOODBANK') and pc.dateCharge='$selectedDate' and pc.paidVia ='Cash' and (pc.timeCharge between '$fromTimez' and '$toTimez') and pc.departmentStatus not like 'remittedBy%%%%%' group by pc.itemNo order by pc.description asc ");
}else {
$result = mysql_query("SELECT pc.*,rd.* FROM patientCharges pc,registrationDetails rd where rd.registrationNo = '$registrationNo' and rd.registrationNo = pc.registrationNo and pc.title='$title' and pc.dateCharge='$selectedDate' and pc.paidVia ='Cash' and (pc.timeCharge between '$fromTimez' and '$toTimez') and pc.departmentStatus not like 'remittedBy%%%%%' group by pc.itemNo order by pc.description asc ");
}
...
echo "<td>&nbsp;<font class='data'><a href='http://".$this->getMyUrl()."/COCONUT/Reports/radiologyReport/radioReportSettings.php?description=$row[description]&registrationNo=$row[registrationNo]&itemNo=$row[itemNo]&branch=Pagadian'>".$row['description']."</a></font>&nbsp;</td>";

}else {

if($row['title'] == "MEDICINE") {
echo "<td>&nbsp;<font class='data'><a href='#'> ".$this->getGeneric($row['chargesCode'])."</a><br>&nbsp;<font size=1 color=red></font></font>&nbsp;</td>";
}else {
...
echo "<font color=red>Cannot Dispense because the patient profile is already locked/discharged</font>";
}
...
$result = mysql_query("SELECT * FROM patientCharges where registrationNo = '$registrationNo' and cashUnpaid > 0 and status not like 'DELETED_%%%%%%'  group by itemNo order by description asc ");
...
echo "<td><center><a href='http://".$this->getMyUrl()."/COCONUT/patientProfile/verifyDelete.php?registrationNo=$registrationNo&show=All&desc=&username=$username&quantity=$row[quantity]&itemNo=$row[itemNo]&description=$row[description]'>
<img src='http://".$this->getMyUrl()."/COCONUT/myImages/delete.jpeg' />
</a></center></td>";
echo "<td>&nbsp;<font class='data'><a href='http://".$this->getMyUrl()."/COCONUT/patientProfile/editCharges.php?itemNo=$row[itemNo]&show=All&desc=&username=$username'>".$row['description']."</a></font>&nbsp;</td>";
...
while($row = mysql_fetch_array($result))
  {
echo "<tr>";
echo "<td>&nbsp;<a href='/Registration/checkingStop.php?patientNo=$row[patientNo]&date=".date("M_d_Y")."'>".$row['completeName']."</a>&nbsp;</td>";
...
public function verifyRecord_alt($name,$username) {
...
while($row = mysql_fetch_array($result))
  {
echo "<tr>";
echo "<td>&nbsp;<a href='/Registration/checkingStop_alt.php?username=$username&patientNo=$row[patientNo]&date=".date("M_d_Y")."'>".$row['completeName']."</a>&nbsp;</td>";
echo "<td>&nbsp;".$row['Birthdate']."&nbsp;</td>";
echo "<td>&nbsp;".$row['Gender']."&nbsp;</td>";
echo "</tr>";
  }
...
$result = mysql_query("SELECT rd.registrationNo,rd.registeredBy,rd.dateRegistered,pr.patientNo,(pr.completeName) as completeName,pr.Birthdate,pr.Gender,pr.lastName,pr.firstName,pr.middleName,pr.contactNo,pr.Birthdate,pr.Gender,pr.Senior,pr.PHIC,pr.civilStatus,pr.Address,pr.phicType FROM patientRecord pr,registrationDetails rd where pr.patientNo = rd.patientNo and pr.completeName like '$name%%%%%%%' group by pr.patientNo ");
...
$result = mysql_query("SELECT inventoryLocation FROM inventoryLocation order by inventoryLocation asc ");
...
$result = mysql_query("SELECT Description FROM room WHERE status = 'Vacant' and branch='$branch' order by Description asc ");
...
public function addNewMedicine($description,$generic,$unitcost,$quantity,$expiration,$addedBy,$dateAdded,$timeAdded,$inventoryLocation,$inventoryType,$branch,$transition,$remarks,$preparation,$phic,$added,$criticalLevel,$supplier,$begCapital,$begQTY,$suppliesUNITCOST) {
...
$sql="INSERT INTO inventory (description,genericName,unitcost,quantity,expiration,addedBy,dateAdded,timeAdded,inventoryLocation,inventoryType,branch,transition,remarks,preparation,phic,Added,criticalLevel,supplier,beginningCapital,beginningQTY,suppliesUNITCOST)
VALUES
('$description','$generic','$unitcost','$quantity','$expiration','$addedBy','$dateAdded','$timeAdded','$inventoryLocation','$inventoryType','$branch','$transition','$remarks','$preparation','$phic','$added','$criticalLevel','$supplier','$begCapital','$begQTY','$suppliesUNITCOST')";
...
echo  "window.location='http://".$this->getMyUrl()."/COCONUT/inventory/addInventory_supplies.php?username=$addedBy '";
}
...
$sql="INSERT INTO inventory (description,genericName,unitcost,quantity,expiration,addedBy,dateAdded,timeAdded,inventoryLocation,inventoryType,branch,transition,remarks,Added)
VALUES
('$description','$generic','$unitcost','$quantity','$expiration','$addedBy','$dateAdded','$timeAdded','$inventoryLocation','$inventoryType','$branch','$transition','$remarks','sellingPrice_0')";
...
if( $this->getPatientRecord_phic() == "YES" && $this->getRegistrationDetails_company() == "" && $row['phic'] == "yes"  ) {
echo "<td>&nbsp;".number_format($priceOption[1] + ($priceOption[1] * 0.15) ,2)."&nbsp;</td>";
$this->med_sp = ($priceOption[1] + ($priceOption[1] * 0.15));
}
//kapag walang philhealth at may HMO/Company
else if( $this->getPatientRecord_phic() == "NO" && $this->getRegistrationDetails_company() != "") {
echo "<td>&nbsp;".number_format($priceOption[1] + ($priceOption[1] * 0.25) ,2)."&nbsp;</td>";
$this->med_sp = ($priceOption[1] + ($priceOption[1] * 0.25));
}
...
echo "<td>&nbsp;<a href='http://".$this->getMyUrl()."/COCONUT/availableMedicine/phicQTY.php?status=UNPAID&registrationNo=$registrationNo&chargesCode=$row[inventoryCode]&description=$row[description]&sellingPrice=".$this->med_sp."&timeCharge=$serverTime&chargeBy=$username&service=Medication&title=MEDICINE&paidVia=Cash&cashPaid=0&batchNo=$batchNo&username=$username&inventoryFrom=$searchFrom&discount=0&room=$room'><font color=green size='2'>PhilHealth</font></a>&nbsp;";
...
$result = mysql_query("SELECT inventoryCode,description,unitcost,quantity FROM inventory WHERE $searchBy like '$searchDesc%%%%%%%' and inventoryType = 'supplies' and inventoryLocation = '$searchFrom' and quantity > 0 and status not like 'DELETED_%%%%%%' and branch = '$branch' order by $searchBy asc ");
...
if( $this->selectNow("registeredUser","module","username",$username) == "PHILHEALTH" || $this->selectNow("registeredUser","module","username",$username) == "HMO" || $this->selectNow("registeredUser","module","username",$username) == "CASHIER" || $this->selectNow("registeredUser","module","username",$username) == "BILLING" || $this->selectNow("registeredUser","module","username",$username) == "PHARMACY" ) {
if( $this->getPatientRecord_phic() == "YES" && $this->getRegistrationDetails_company() == "" ) { //phic:yes; hmo:no;
echo "<td>&nbsp;".($row['unitcost'] + ($row['unitcost'] * 0.15))."&nbsp;</td>";
}
else if( $this->getPatientRecord_phic() == "NO" && $this->getRegistrationDetails_company() != "" ) { //phic:no; hmo:yes;
echo "<td>&nbsp;".($row['unitcost'] + ($row['unitcost'] * 0.25))."&nbsp;</td>";
}
else if( $this->getPatientRecord_phic() == "YES" && $this->getRegistrationDetails_company() != "" ) { //phic:yes; hmo:yes;
echo "<td>&nbsp;".($row['unitcost'] + ($row['unitcost'] * 0.25))."&nbsp;</td>";
}
else {
...
VALUES
('$username','$password','$module','$branch','".mysql_real_escape_string($completeName)."')";
...
//$result = mysql_query("SELECT upper(pr.completeName) as completeName, pr.patientNo FROM patientRecord pr, registrationDetails rd WHERE pr.patientNo = rd.patientNo and (pr.completeName like '$completeName%%%%%' or rd.registrationNo = '$completeName') and pr.firstName != 'N/A' order by pr.lastName");
$result = mysql_query("SELECT upper(completeName) as completeName, patientNo FROM patientRecord WHERE completeName like '$completeName%%%%%' and firstName != 'N/A' order by lastName");

while($row = mysql_fetch_array($result))
  {
    echo $row['completeName']."_".$row['patientNo']."\n";
  }
...
$result = mysql_query("SELECT rd.dateUnregistered,rd.patientNo,pr.completeName,rd.registrationNo,rd.dateRegistered,rd.branch,rd.type FROM patientRecord pr,registrationDetails rd WHERE pr.patientNo = rd.patientNo and rd.patientNo = '$completeName' order by rd.control_dateRegistered desc limit $start,$end ");
...
echo "<td class='myData'><center>".$row['patientNo']."</center></td>";
...
$result = mysql_query("SELECT rd.dateUnregistered,rd.patientNo,pr.completeName,rd.registrationNo,rd.dateRegistered,rd.branch,rd.type FROM patientRecord pr,registrationDetails rd WHERE pr.patientNo = rd.patientNo and rd.patientNo = '$completeName' order by rd.control_dateRegistered desc ");
...
if($module=="PHARMACY" || $module =="CSR") {
$result = mysql_query("SELECT rd.type,rd.registrationNo,upper(pr.lastName) as lastName,upper(pr.firstName) as firstName,sum(pc.cashUnpaid) as grandTotal FROM patientRecord pr,registrationDetails rd,patientCharges pc WHERE pr.patientNo = rd.patientNo and rd.registrationNo = pc.registrationNo and pc.dateCharge = '$dateSelected' and (pc.timeCharge between '$fromTimez' and '$toTimez') and pc.inventoryFrom='$module' and pc.status not like 'DELETED_%%%%%%%' and pc.departmentStatus not like 'dispensedBy%%%%' and rd.branch='$branch' group by rd.registrationNo order by pr.lastName asc ");

}else if( $module == "LABORATORY" ) {
$result = mysql_query("SELECT rd.type,rd.registrationNo,upper(pr.lastName) as lastName,upper(pr.firstName) as firstName,sum(pc.total) as grandTotal FROM patientRecord pr,registrationDetails rd,patientCharges pc WHERE pr.patientNo = rd.patientNo and rd.registrationNo = pc.registrationNo and pc.dateCharge = '$dateSelected' and (pc.timeCharge between '$fromTimez' and '$toTimez') and (pc.title='$module' or pc.title='BLOODBANK') and pc.departmentStatus not like 'remittedBy%%%%' and rd.branch='$branch' group by rd.registrationNo order by pc.itemNo desc ");
}else {
$result = mysql_query("SELECT pc.description,pc.itemNo,rd.type,rd.registrationNo,upper(pr.lastName) as lastName,upper(pr.firstName) as firstName,sum(pc.total) as grandTotal FROM patientRecord pr,registrationDetails rd,patientCharges pc WHERE pr.patientNo = rd.patientNo and rd.registrationNo = pc.registrationNo and pc.dateCharge = '$dateSelected' and (pc.timeCharge between '$fromTimez' and '$toTimez') and pc.title='$module' and pc.departmentStatus not like 'remittedBy%%%%' and rd.branch='$branch' group by rd.registrationNo order by pr.lastName asc ");
}
...
$result = mysql_query("SELECT upper(pr.completeName) as completeName,pc.description,pc.sellingPrice,pc.quantity,pc.discount,pc.total,pc.cashUnpaid,pc.cashPaid,pc.paidBy,pc.paidVia FROM patientRecord pr,registrationDetails rd,patientCharges pc WHERE pr.patientNo = rd.patientNo and rd.registrationNo = pc.registrationNo and pc.datePaid = '$dateSelected' and (pc.timePaid between '$fromTimez' and '$toTimez') and (pc.status='PAID') and pc.paidBy='$username' group by pc.itemNo order by pc.title,completeName asc ");
}
...
//$result = mysql_query("SELECT sum(cashUnpaid) as total from patientCharges where registrationNo = '$registrationNo' and title IN ('LABORATORY','RADIOLOGY','MEDICINE','SUPPLIES','REHAB','NBS','ECG','Room And Board','MISCELLANEOUS','OR/DR/ER Fee','OXYGEN','NURSING-CHARGES','CARDIAC','OVERTIME','BLOODBANK','GENERATOR_CHARGE') and status = 'UNPAID' ");
$result = mysql_query("SELECT sum(cashUnpaid) as total from patientCharges where registrationNo = '$registrationNo' and title NOT LIKE 'PROFESSIONAL FEE' and  (status = 'UNPAID' or status = 'Discharged') ");
}
...
if( $this->selectNow("registeredUser","module","username",$username) == "ADMIN" ) {
$result = mysql_query("SELECT rd.registrationNo,pp.paidVia,upper(pr.completeName) as completeName,pp.paymentFor,pp.paidBy,pp.datePaid,pp.amountPaid,pp.pf,pp.admitting FROM patientPayment pp,patientRecord pr,registrationDetails rd,patientCharges pc WHERE pr.patientNo = rd.patientNo and pp.registrationNo = rd.registrationNo and rd.registrationNo = pc.registrationNo and pp.datePaid = '$dateSelected' and (pp.timePaid between '$fromTimez' and '$toTimez') and paymentFor not in ('CUT-OFF HOSPITAL BILL') group by paymentNo order by completeName asc ");
}else {
$result = mysql_query("SELECT rd.registrationNo,pp.paidVia,upper(pr.completeName) as completeName,pp.paymentFor,pp.paidBy,pp.datePaid,pp.amountPaid,pp.pf,pp.admitting FROM patientPayment pp,patientRecord pr,registrationDetails rd,patientCharges pc WHERE pr.patientNo = rd.patientNo and pp.registrationNo = rd.registrationNo and rd.registrationNo = pc.registrationNo and pp.datePaid = '$dateSelected' and (pp.timePaid between '$fromTimez' and '$toTimez') and paymentFor not in ('CUT-OFF HOSPITAL BILL') and pp.paidBy='$username' group by paymentNo order by completeName asc ");
}
...
mysql_select_db($this->database, $con);
if( $type == "OPD" ) {
$result = mysql_query("SELECT rd.registeredBy,rd.type,rd.registrationNo,upper(pr.lastName) as lastName,upper(pr.firstName) as firstName,sum(pc.cashUnpaid) as total FROM patientRecord pr,registrationDetails rd,patientCharges pc WHERE pr.patientNo = rd.patientNo and rd.registrationNo = pc.registrationNo and rd.dateUnregistered = '' and pc.branch='$branch' and pc.dateCharge = '$dateSelected' and (rd.type='OPD' or rd.type='DIALYSIS') group by rd.registrationNo order by pr.lastName asc   ");

}else if( $type == "IPD" ) {
$result = mysql_query("SELECT rd.registeredBy,rd.type,rd.registrationNo,upper(pr.lastName) as lastName,upper(pr.firstName) as firstName,sum(pc.cashUnpaid) as total FROM patientRecord pr,registrationDetails rd,patientCharges pc WHERE pr.patientNo = rd.patientNo and rd.registrationNo = pc.registrationNo and rd.dateUnregistered = '' and pc.branch='$branch' and pc.dateCharge = '$dateSelected' and rd.type='IPD' group by rd.registrationNo order by pr.lastName asc   ");

}else {
$result = mysql_query("SELECT rd.registeredBy,rd.type,rd.registrationNo,upper(pr.lastName) as lastName,upper(pr.firstName) as firstName,sum(pc.cashUnpaid) as total FROM patientRecord pr,registrationDetails rd,patientCharges pc WHERE pr.patientNo = rd.patientNo and rd.registrationNo = pc.registrationNo and rd.dateUnregistered = '' and pc.branch='$branch' and pc.dateCharge = '$dateSelected' group by rd.registrationNo order by pr.lastName asc   ");
}
echo "<table border=1 cellpadding=0 cellspacing=0>";
echo "<tr>";
...
$result = mysql_query("SELECT service,description,total,company,phic,cashPaid,cashUnpaid from patientCharges where registrationNo = '$registrationNo' and title = 'PROFESSIONAL FEE' and status = 'UNPAID' ");
...
echo "<td><div align='left'><span class='Arial11BlackBold'>&nbsp;".$row['description']."&nbsp;</span><br><span class='Arial11Red'>( ".$row['service']." )</span></div></td>";
echo "<td><div align='right' class='Arial13Black'>&nbsp;".number_format($row['total'],2)."&nbsp;</div></tD>";
echo "<td><div align='right' class='Arial13Black'>&nbsp;".number_format($row['phic'],2)."&nbsp;</div></tD>";
echo "<td><div align='right' class='Arial13Black'>&nbsp;".number_format($row['company'],2)."&nbsp;</div></tD>";
//echo "<td>&nbsp;<font size=2>".$row['phic']."</font></tD>";
echo "<td><div align='right' class='Arial13Black'>&nbsp;".number_format($row['cashUnpaid'],2)."&nbsp;</div></tD>";
...
$result = mysql_query("SELECT description,total,company,phic,cashPaid,cashUnpaid from patientCharges where registrationNo = '$registrationNo' and title = 'PROFESSIONAL FEE' and status = 'UNPAID' ");
...
$result = mysql_query("SELECT sum(total) as total from patientCharges where registrationNo = '$registrationNo' and title = 'Room And Board'  ");
...
$result = mysql_query("SELECT requestingBranch,requestingDepartment,requestTo_department,requestTo_branch from inventoryManager WHERE requestTo_department='$myDepartment' and requestTo_branch = '$myBranch' group by requestingBranch,requestingDepartment  ");
...
$this->coconutTableHeader("UNIT COST");
...
if($show == "All") {
$result = mysql_query("SELECT * from inventory where (inventoryType = '$inventoryType' or inventoryLocation='$inventoryType') and branch='$branch' order by description asc  ");
}else {
$result = mysql_query("SELECT * from inventory where description like '$desc%%%%%' and (inventoryType = '$inventoryType' or inventoryLocation='$inventoryType') and branch='$branch' order by description asc  ");
}
...
echo "<th>&nbsp;<font class='data'>Beginning Capital</font>&nbsp;</th>";
...
echo "<td>&nbsp;<font class='data'>".$row['beginningCapital']."</font>&nbsp;</td>";
...
echo "<td>&nbsp;".$row['completeName']."&nbsp;</td>";
echo "<td>&nbsp;".$row['proficiencyNo']."&nbsp;</td>";
echo "<td>&nbsp;<a href='http://".$this->getMyUrl()."/COCONUT/masterfile/EDIT/editUser.php?username=$username&user=$row[username]&password=$row[password]&module=$row[module]&branch=$row[branch]&completeName=$row[completeName]&proficiencyNo=$row[proficiencyNo]&employeeID=$row[employeeID]&show=$show'><img src='http://".$this->getMyUrl()."/COCONUT/myImages/pencil.jpeg'></a>&nbsp;</td>";
...
public function getMasterListInventory_requesting($inventoryType,$username,$branch,$description,$requestingDepartment,$requestNo) {
...
mysql_select_db($this->database, $con);
if($branch == "All") {
$result = mysql_query("SELECT * from inventory where inventoryType = '$inventoryType' and inventoryLocation='PHARMACY' and description like '$description%%%%%%' and branch='$branch' and quantity > 0 order by description asc  ");
}else {
$result = mysql_query("SELECT * from inventory where inventoryType = '$inventoryType' and inventoryLocation='PHARMACY' and description like '$description%%%%%%%' and branch='$branch' and quantity > 0 order by description asc  ");
}
echo "<center><table border=1 cellpadding=0 cellspacing=0>";
...
echo "<tr>";
echo "<td>&nbsp;<font class='data'>".$row['inventoryCode']."</font>&nbsp;</td>";
echo "<td>&nbsp;<font class='data'><a href='http://".$this->getMyUrl()."/COCONUT/availableMedicine/medicineRequest2.php?description=$row[description]&requestTo_department=PHARMACY&branch=$branch&requestingDepartment=$requestingDepartment&inventoryCode=$row[inventoryCode]&username=$username&requestNo=$requestNo'>".$row['description']."</a></font>&nbsp;</td>";
echo "<td>&nbsp;<font class='data'>".$row['genericName']."</font>&nbsp;</td>";
echo "<td>&nbsp;<font class='data'>".$row['unitcost']."</font>&nbsp;</td>";
echo "<td>&nbsp;<font class='data'>".$row['quantity']."</font>&nbsp;</td>";
echo "<td>&nbsp;<font class='data'>".$row['expiration']."</font>&nbsp;</td>";
...
$result = mysql_query("SELECT count(verificationNo) as request FROM inventoryManager WHERE requestTo_department = '$requestTo_department' and requestTo_branch = '$requestTo_branch' and requestingDepartment='$requestingDepartment' and requestingBranch = '$requestingBranch' and status='requesting'  ");
...
$result = mysql_query("SELECT itemNo from core2_laboratoryResultChecker where itemNo='$itemNo'  ");
...
$result = mysql_query("SELECT itemNo from radioSavedReport where itemNo='$itemNo'  ");
...
$result = mysql_query("SELECT * from inventory where inventoryLocation='$module' and branch='$branch' and status not like 'DELETED_%%%%%%' order by description asc  ");
...
mysql_select_db($this->database, $con);
$date = date("M_d_Y");
$result = mysql_query("SELECT *,pr.completeName,pc.registrationNo from inventory i,patientCharges pc,registrationDetails rd,patientRecord pr where i.inventoryLocation='$module' and pc.branch='$branch' and i.inventoryCode = pc.chargesCode and pc.registrationNo = rd.registrationNo and rd.patientNo = pr.patientNo and (pc.title = 'MEDICINE' or pc.title='SUPPLIES') and pc.departmentStatus like 'dispensedBy%%%%%' and pc.dateCharge = '$myDate' order by pc.departmentStatus_time asc  ");
echo "<font size=2>Current Inventory Usages in $module of $branch </font><br><br>";
echo "<center><table border=1 cellpadding=0 cellspacing=0>";
echo "<tr>";
echo "<th>&nbsp;<font class='data'>Name</font>&nbsp;</th>";
echo "<th>&nbsp;<font class='data'>Description</font>&nbsp;</th>";
...
echo "<th>&nbsp;<font class='data'>Attending Physician</font>&nbsp;</th>";
...
echo "<td>&nbsp;<font class='data'>".$this->getAttendingDoc($row['registrationNo'],"ATTENDING")."</font>&nbsp;</td>";
...
$result = mysql_query("SELECT rd.registrationNo,upper(pr.lastName) as lastName,upper(pr.firstName) as firstName,pc.service,pc.sellingPrice,pc.cashPaid from patientRecord pr,registrationDetails rd,patientCharges pc WHERE pr.patientNo = rd.patientNo and rd.registrationNo = pc.registrationNo and pc.description ='$doctor' and pc.dateCharge like '$year-$month-$day' and (pc.timeCharge between '$fromTime_hour:$fromTime_minutes:$fromTime_seconds' and '$toTime_hour:$toTime_minutes:$toTime_seconds') and rd.type='OPD' order by lastName asc    ");
...
public function getDoctorPFReport($type,$username,$month,$day,$year,$fromTime_hour,$fromTime_minutes,$fromTime_seconds,$toTime_hour,$toTime_minutes,$toTime_seconds,$show) {
...
if( $show == "All" ) {
$result = mysql_query("SELECT pc.description,rd.registrationNo,upper(pr.lastName) as lastName,upper(pr.firstName) as firstName,pc.service,pc.sellingPrice from patientRecord pr,registrationDetails rd,patientCharges pc WHERE pr.patientNo = rd.patientNo and rd.registrationNo = pc.registrationNo and pc.dateCharge = '$date' and (pc.timeCharge between '$fromTime_hour:$fromTime_minutes:$fromTime_seconds' and '$toTime_hour:$toTime_minutes:$toTime_seconds') and title = 'PROFESSIONAL FEE' and rd.type='$type' order by pc.description,pr.lastName asc    ");
}else {
$result = mysql_query("SELECT pc.description,rd.registrationNo,upper(pr.lastName) as lastName,upper(pr.firstName) as firstName,pc.service,pc.sellingPrice from patientRecord pr,registrationDetails rd,patientCharges pc WHERE pr.patientNo = rd.patientNo and rd.registrationNo = pc.registrationNo and pc.dateCharge = '$date' and (pc.timeCharge between '$fromTime_hour:$fromTime_minutes:$fromTime_seconds' and '$toTime_hour:$toTime_minutes:$toTime_seconds') and title = 'PROFESSIONAL FEE' and pc.description='$show' and rd.type='$type' order by pc.description,pr.lastName asc    ");
}
...
$result = mysql_query("SELECT sum(pc.sellingPrice) as sellingPrice from patientCharges pc,registrationDetails rd WHERE rd.registrationNo = pc.registrationNo and pc.description = '$doctor' and title = 'PROFESSIONAL FEE' and (rd.dateUnregistered between '$fromDate' and '$toDate') and pc.branch = '$branch' and rd.type='IPD'  ");
}else {}
...
where (lsr.control_date between '$fromDate' and '$toDate') and lsr.chargesCode = '$chargesCode' and rd.type='$type'
");
...
$result = mysql_query("SELECT Description,chargesCode,OPD from availableCharges WHERE Category = '$title' order by Description asc  ");
while($row = mysql_fetch_array($result))
  {
$this->listExaminationAsRow_total = ( $this->getResultPatient($title,"IPD",$row['chargesCode'],$fromDate,$toDate) + $this->getResultPatient($title,"OPD",$row['chargesCode'],$fromDate,$toDate) + $this->getResultPatient($title,"ER",$row['chargesCode'],$fromDate,$toDate) +  $this->getResultPatient($title,"DIALYSIS",$row['chargesCode'],$fromDate,$toDate) );
echo "<tr id='rowz'>";
...
echo "<td>&nbsp;<font color=black>".$this->getResultPatient($title,"DIALYSIS",$row['chargesCode'],$fromDate,$toDate)."</font></td>";
...
$result = mysql_query("SELECT sum(pc.company) as total from patientCharges pc,registrationDetails rd WHERE rd.registrationNo = pc.registrationNo and rd.Company = '$hmo' and rd.type='$type' and pc.company > 0 and (pc.dateCharge between '$fromDate' and '$toDate') and pc.branch = '$branch'  ");
...
$result = mysql_query("SELECT sum(pc.quantity) as qty,sum(pc.total) as total,sum(pc.phic) as phic,sum(pc.cashUnpaid) as cashUnpaid,sum(company) as company,sum(cashPaid) as cashPaid from patientCharges pc WHERE pc.registrationNo='$registrationNo' and pc.chargesCode = '$chargesCode' "); 
...
if($show == "All") {
$result = mysql_query("SELECT pc.*,rd.* from patientCharges pc,registrationDetails rd WHERE rd.registrationNo = '$registrationNo' and pc.registrationNo = rd.registrationNo order by pc.title asc  ");
}else if($show == "try") {
...
$result = mysql_query("SELECT pc.*,rd.* from patientCharges pc,registrationDetails rd WHERE rd.registrationNo = '$registrationNo' and rd.registrationNo = pc.registrationNo and (dateCharge between '$data1' and '$data2') order by pc.description asc  ");
}else if($show == "Category") {
$result = mysql_query("SELECT pc.*,rd.* from patientCharges pc,registrationDetails rd WHERE rd.registrationNo = '$registrationNo' and rd.registrationNo = pc.registrationNo and title='$data1' order by description asc  ");
}else if($show == "phic") {
$result = mysql_query("SELECT pc.*,rd.* from patientCharges pc,registrationDetails rd WHERE rd.registrationNo = '$registrationNo' and rd.registrationNo = pc.registrationNo and phic > 0 order by description asc  ");
}else if($show == "company") {
$result = mysql_query("SELECT pc.*,rd.* from patientCharges pc,registrationDetails rd WHERE rd.registrationNo = '$registrationNo' and rd.registrationNo = pc.registrationNo and pc.company > 0 order by pc.description asc  ");
}else if($show == "cashPaid") {
$result = mysql_query("SELECT pc.*,rd.* from patientCharges pc,registrationDetails rd WHERE rd.registrationNo = '$registrationNo' and rd.registrationNo = pc.registrationNo and cashPaid > 0 order by description asc  ");
}else if($show == "cashUnpaid") {
$result = mysql_query("SELECT pc.*,rd.* from patientCharges pc,registrationDetails rd WHERE rd.registrationNo = '$registrationNo' and rd.registrationNo = pc.registrationNo and cashUnpaid > 0 order by description asc  ");
}else if($show == "selected") {
$result = mysql_query("SELECT pc.*,rd.* from patientCharges pc,registrationDetails rd WHERE rd.registrationNo = '$registrationNo' and rd.registrationNo = pc.registrationNo and itemNo='$data1' order by description asc  ");
}else {
echo "";
}
echo "<Table border=1 cellpadding=0 cellspacing=0 rules=all>";
...
$result = mysql_query("SELECT pc.*,rd.* from patientCharges pc,registrationDetails rd WHERE rd.registrationNo = '$registrationNo' and pc.registrationNo = rd.registrationNo and pc.status = 'UNPAID' order by description asc  ");
...
if($show == "All") {
$result = mysql_query("SELECT pc.*,rd.* from patientCharges pc,registrationDetails rd WHERE rd.registrationNo = '$registrationNo' and pc.registrationNo = rd.registrationNo order by title asc  ");
}

else if($show == "try") {
$result = mysql_query("SELECT pc.chargesCode,pc.quantity,pc.description,pc.quantity,pc.sellingPrice,pc.dateCharge,pc.discount,pc.total,pc.cashPaid,pc.company,pc.phic,pc.cashUnpaid,pc.title,rd.* from patientCharges pc,registrationDetails rd WHERE rd.registrationNo = '$registrationNo' and pc.registrationNo = rd.registrationNo and status = 'UNPAID' order by pc.title asc  ");
}
...
$result = mysql_query("SELECT pc.*,rd.* from patientCharges pc,registrationDetails rd WHERE rd.registrationNo = '$registrationNo' and rd.registrationNo = pc.registrationNo and title='$data1' and status = 'UNPAID' order by description asc  ");
}else if($show == "phic") {
$result = mysql_query("SELECT pc.*,rd.* from patientCharges pc,registrationDetails rd WHERE rd.registrationNo = '$registrationNo' and rd.registrationNo = pc.registrationNo and phic > 0 order by description asc  ");
}else if($show == "company") {
$result = mysql_query("SELECT pc.*,rd.* from patientCharges pc,registrationDetails rd WHERE rd.registrationNo = '$registrationNo' and rd.registrationNo = pc.registrationNo and pc.company > 0 order by pc.description asc  ");
...
if($switch == 1) {
$result = mysql_query("SELECT sum(pc.total) as total,sum(pc.phic) as totalPHIC from patientCharges pc WHERE pc.title = 'MEDICINE' and pc.registrationNo = '$identificationNo' and pc.status = 'UNPAID' and pc.departmentStatus != '' "); 

}else {
$result = mysql_query("SELECT sum(pc.total) as total,sum(pc.phic) as totalPHIC from patientCharges pc WHERE pc.title = 'MEDICINE' and pc.itemNo = '$identificationNo' and pc.departmentStatus != '' "); 
}
while($row = mysql_fetch_array($result))
  {
...
$result = mysql_query("SELECT sum(pc.total) as total,sum(pc.phic) as totalPHIC from patientCharges pc WHERE  pc.registrationNo = '$registrationNo' and (pc.title != 'MEDICINE' and pc.title != 'Room And Board' and pc.title != 'PROFESSIONAL FEE' and pc.title != 'NURSING-CHARGES' and pc.title != 'MISCELLANEOUS' ) "); 
...
echo "<Td><input type=text class='desc' value='".$this->selectNow("inventory","genericName","inventoryCode",$row['chargesCode'])."/".$row['description']."'></td>";
echo "<Td><input type=text class='preparation' value='".$this->selectNow("inventory","preparation","inventoryCode",$row['chargesCode'])."'></td>";
...
if( $title == "SUPPLIES" ) {
$result = mysql_query("SELECT pc.* from patientCharges pc WHERE pc.title = '$title' and pc.registrationNo = '$registrationNo' and phic > 0 and pc.status = 'UNPAID' and departmentStatus like 'dispensedBy_%%%%%%' group by pc.description order by description asc "); 
}else {
...
echo "<Td><input type=text class='qtyRadio' value='".$this->phicBack_3_qty()."'></tD>";
...
$result = mysql_query("SELECT * from patientICD where registrationNo='$registrationNo' order by icdCode asc  ");
...
if($type == "All") {
$result = mysql_query("SELECT * FROM patientCharges where registrationNo = '$registrationNo' and (status = 'UNPAID' or status = 'Discharged') order by description asc ");
}else {
$result = mysql_query("SELECT * FROM patientCharges where registrationNo = '$registrationNo' and (status = 'UNPAID' or status = 'Discharged') and title = '$type' order by description asc ");
}
...
$result = mysql_query("SELECT * FROM patientCharges where registrationNo = '$registrationNo' and description like '$desc%%%%%%' order by description asc ");
}
...
echo "<th bgcolor='#3b5998'>&nbsp;<font color=white>Admitting</font>&nbsp;</th>";
...
public function descPartialPayment($registrationNo) {
...
$result = mysql_query("SELECT paymentFor,( amountPaid + pf + admitting ) as total FROM patientPayment WHERE registrationNo = '$registrationNo' ");
...
echo "<Td>&nbsp;<a href='http://".$this->getMyUrl()."/Department/redirect.php?username=$username&registrationNo=".$patientData[0]."' target='_blank' class='Exceed'><font class='roomData'>".$patientData[1]."</font></a>&nbsp;</td>";
}else {
...
$result = mysql_query("select upper(pr.lastName) as lastName,upper(pr.firstName) as firstName,rd.dateRegistered,rd.dateUnregistered,rd.registrationNo from patientRecord pr,registrationDetails rd where pr.patientNo = rd.patientNo and rd.type='IPD' and rd.branch = '$branch' and (rd.dateUnregistered between '$fromDate' and '$toDate') order by pr.lastName ");
}
echo "<center>";
...
$result = mysql_query("SELECT pc.* FROM patientCharges pc where pc.registrationNo='$registrationNo' and pc.batchNo='$batchNo' and pc.departmentStatus not like 'dispensedBy_%%%%%%%' and pc.status not like 'DELETED_%%%%%%%%' order by pc.description asc  ");
...
$result = mysql_query("SELECT sum(company) as total FROM patientCharges where registrationNo = '$registrationNo' and title != 'PROFESSIONAL FEE' ");
...
$result = mysql_query("SELECT upper(pr.lastName) as lastName,upper(pr.firstName) as firstName,rd.*,pp.* from patientRecord pr,registrationDetails rd,patientPayment pp WHERE pr.patientNo = rd.patientNo and rd.registrationNo = pp.registrationNo and (pp.datePaid between '$fromDate' and '$toDate') order by rd.branch asc  "); 
...
if( $show == "All" ) {
$result = mysql_query("SELECT upper(pr.lastName) as lastName,upper(pr.firstName) as firstName,rd.*,pc.* FROM patientRecord pr,registrationDetails rd,patientCharges pc WHERE pr.patientNo = rd.patientNo and rd.registrationNo = pc.registrationNo and (rd.dateUnregistered between '$fromDate' and '$toDate') and pc.title = 'PROFESSIONAL FEE' order by pc.description,pr.lastName asc ");
}else {
$result = mysql_query("SELECT upper(pr.lastName) as lastName,upper(pr.firstName) as firstName,rd.*,pc.* FROM patientRecord pr,registrationDetails rd,patientCharges pc WHERE pr.patientNo = rd.patientNo and rd.registrationNo = pc.registrationNo and (rd.dateUnregistered between '$fromDate' and '$toDate') and type='IPD' and pc.title = 'PROFESSIONAL FEE' and pc.description='$show' order by pc.description,pr.lastName asc ");
}
...
echo "<tr id='rowz'>";
//$price = preg_split ("/\//", $row['sellingPrice']); 
$this->doctorPF_ipd_price += $row['sellingPrice'];
$this->doctorPF_ipd_pf += $row['sellingPrice'];
$this->coconutTableData($row['lastName'].", ".$row['firstName']);
$this->coconutTableData($row['branch']);
$this->coconutTableData($row['dateRegistered']." - ".$row['dateUnregistered']);
$this->coconutTableData($row['description']);
...
public function censusRegistered($month,$day,$year,$month1,$day1,$year1,$type,$branch,$username) {
...
if($type == "IPD") {
$result = mysql_query("SELECT pr.patientNo,upper(pr.lastName) as lastName,upper(pr.firstName) as firstName,upper(pr.middleName) as middleName,pr.Birthdate,pr.Age,rd.* FROM patientRecord pr,registrationDetails rd WHERE pr.patientNo = rd.patientNo and (dateRegistered between '$fromRegistered' and '$toRegistered' ) and rd.type in ('IPD','ER','OR/DR') order by dateRegistered,timeRegistered asc ");
}else {
...
echo "<br><center>";
echo "
<table border='1' cellpadding='0' rules='all' cellspacing='0'>
<tr>
<td bgcolor='#3b5998'><div align='center' class='Arial12WhiteBold'>&nbsp;Reg. Number&nbsp;</div></td>
<td bgcolor='#3b5998'><div align='center' class='Arial12WhiteBold'>&nbsp;Name&nbsp;</div></td>
<td bgcolor='#3b5998'><div align='center' class='Arial12WhiteBold'>&nbsp;Date of Birth&nbsp;</div></td>
<td bgcolor='#3b5998'><div align='center' class='Arial12WhiteBold'>&nbsp;Age&nbsp;</div></td>
<td bgcolor='#3b5998'><div align='center' class='Arial12WhiteBold'>&nbsp;Attending Physician&nbsp;</div></td>
<td bgcolor='#3b5998'><div align='center' class='Arial12WhiteBold'>&nbsp;Date/Time Registered&nbsp;</div></td>
<td bgcolor='#3b5998'><div align='center' class='Arial12WhiteBold'>&nbsp;Registered By&nbsp;</div></td>
...
$bday=preg_split("/_/",$row['Birthdate']);
$bdayfmt=$bday[2]."-".$bday[0]."-".$bday[1];

echo "
<td><div align='left' class='Arial15Black'>&nbsp;".$row['registrationNo']."&nbsp;</div></td>
<td><div align='left' class='Arial15Black'>&nbsp;".$row['lastName']." ".$row['firstName']." ".$row['middleName']."&nbsp;</div></td>
<td><div align='center' class='Arial15Black'>&nbsp;".date("M d, Y",strtotime($bdayfmt))."&nbsp;</div></td>
<td><div align='center' class='Arial15Black'>&nbsp;".$row['Age']."&nbsp;</div></td>
<td><div align='left' class='Arial15Black'>&nbsp;".$this->getAttendingDoc($row['registrationNo'],"Attending")."&nbsp;</div></td>
<td><div align='center' class='Arial12Black'>&nbsp;".date("M d, Y",strtotime($row['dateRegistered']))." - ".$row['timeRegistered']."&nbsp;</div></td>
<td><div align='center' class='Arial15Black'>&nbsp;".$row['registeredBy']."&nbsp;</div></td>
<td><div align='center' class='Arial15Black'><a href='http://".$this->getMyUrl()."/COCONUT/Reports/Census/registrationCensusDelete.php?username=$username&year=$year&year1=$year1&month=$month&month1=$month1&day=$day&day1=$day1&branch=$branch&type=$type&registrationNo=$row[registrationNo]'><img src='http://".$this->getMyUrl()."/COCONUT/myImages/delete.jpeg'></a></div></td>
";
echo "</tr>";
  }
echo "
<tr>
<td colspan='6'><b>&nbsp;TOTAL PATIENT</b></td>
<td><div align='center'><b>".$this->censusRegistered_patient."</b></div></td>
<td></td>
</tr>
";
...
$result = mysql_query("SELECT noteBy,noteMessage,date FROM patientNotes where noteType = 'guest' order by noteNo desc ");
...
$result = mysql_query("SELECT * from inventory where (expiration between '$fromDate' and '$toDate') order by description asc  ");
...
if($title != "") {
$result = mysql_query("SELECT sum($cols) as cols FROM patientCharges where registrationNo = '$registrationNo' and title='$title' and status = 'UNPAID' ");
}else {
$result = mysql_query("SELECT sum($cols) as cols FROM patientCharges where registrationNo = '$registrationNo' and status = 'UNPAID' ");
}
...
if($title != "") {
$result = mysql_query("SELECT sum($cols) as cols,sum(cashPaid) as paid,status FROM patientCharges where registrationNo = '$registrationNo' and title='$title' and status = 'UNPAID' ");
}else {
...
public function getTotal_hdu_paid($cols,$title,$registrationNo) {
...
if($title != "") {
$result = mysql_query("SELECT sum($cols) as cols,cashPaid as paid,status FROM patientCharges where registrationNo = '$registrationNo' and title='$title' and (status = 'PAID' or (cashUnpaid < 1 and company > 0)) and status not like 'DELETED_%%%%%%' ");
}else {
$result = mysql_query("SELECT sum($cols) as cols,sum(cashPaid) as paid FROM patientCharges where registrationNo = '$registrationNo' and status not like 'DELETED_%%%%%%%%%%' ");
}
...
$result = mysql_query(" SELECT rd.registrationNo,pr.lastName,pr.firstName,rd.room FROM registrationDetails rd,patientRecord pr WHERE rd.patientNo = pr.patientNo and (rd.dateUnregistered between '$fromDate' and '$toDate') and rd.branch = '$branch'  ");
...
$result = mysql_query(" SELECT description,genericName,quantity,criticalLevel FROM inventory where criticalLevel >= quantity and criticalLevel != 0  ");
...
$result = mysql_query("SELECT description from patientCharges where registrationNo = '$registrationNo' and service like '%$service%' and status not like 'DELETED_%%%%%%%%'  ");
...
public function individual_doc_PF($name,$month,$day,$year) {
...
$result = mysql_query("SELECT rd.registrationNo,rd.dateUnregistered,pr.lastName,pr.firstName,pc.sellingPrice,pc.service,pc.dateCharge,pc.timeCharge,pc.cashUnpaid FROM patientCharges pc,registrationDetails rd,patientRecord pr WHERE pc.registrationNo = rd.registrationNo and rd.patientNo = pr.patientNo and pc.description = '$name' and rd.dateUnregistered = '$datez' and rd.type='IPD' order by pr.lastName   ");
...
while($row = mysql_fetch_array($result))
  {
echo "<font size=3>".$row['diagnosis'].",</font>";
  }
echo "</td>";
}
...
public function alreadyPay($registrationNo) { //mga patient n bill pru may bbyran p rin sa cashier 
...
$result = mysql_query("SELECT sum(amountPaid) as cazh from patientPayment where registrationNo = '$registrationNo' ");
...
public function insertInventoryZero($inventoryCode,$description,$time,$date,$user,$px,$itemNo) {
...
$sql="INSERT INTO inventoryZero (inventoryCode,description,time,date,lastDispensedUser,lastDispensedPx,itemNo)
VALUES
('$inventoryCode','$description','$time','$date','$user','$px','$itemNo')";
